...
export default function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // --- Phone Configuration ---
  const PHONE_NUMBER = "94777752858"; // The number you provided

  // --- Handle Form Submit (Sends via WhatsApp) ---
...
*Message:* ${formData.message}`;
  ...
  // --- Handle Direct Chat Button ---
  const handleDirectChat = () => {
    const text = `Hi, I am interested in the ${gem.title} (ID: ${gem.sku}). Is it still available?`;
    const url = `https://wa.me/${PHONE_NUMBER}?text=${encodeURIComponent(text)}`;
    window.open(url, '_blank');
  };
          ...
          <div className="md:col-span-5">
            ...
            <div className="bg-white border border-gray-200 p-6 rounded-lg shadow-sm mb-8">
              ...
              <form onSubmit={handleFormSubmit} className="space-y-4">
                ...
                <button type="submit" className="w-full bg-black text-white py-3 font-bold uppercase text-sm hover:bg-gray-800 transition">
                  Submit Inquiry
                </button>
              </form>
            </div>
