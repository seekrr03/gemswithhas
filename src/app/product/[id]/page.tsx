...
export default function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const PHONE_NUMBER = "94777752858"; 
...
*Message:* ${formData.message}`;
  ...
  const handleDirectChat = () => {
    const text = `Hi, I am interested in the ${gem.title} (ID: ${gem.sku}).`;
    const url = `https://wa.me/${PHONE_NUMBER}?text=${encodeURIComponent(text)}`;
    window.open(url, '_blank');
  };

  const handleBuyNow = () => {
    const text = `*BUY ORDER REQUEST*
-------------------------
...
Please send payment details.`;
  ...
  return (
    <div className="min-h-screen bg-white text-gray-800 font-sans pb-20 relative">
      
      {/* 1. SIMPLE NAVIGATION (Breadcrumbs) */}
      <nav className="border-b border-gray-200 px-4 md:px-6 py-4 flex flex-wrap items-center gap-2 text-xs md:text-sm uppercase tracking-wide text-gray-500">
        <Link href="/" className="hover:text-black">Home</Link> 
        <span>/</span>
        <Link href="/#collection" className="hover:text-black">Gemstones</Link>
        <span>/</span>
      ...
      <main className="max-w-7xl mx-auto px-4 mt-6 md:mt-8">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-10">
          ...
          {/* LEFT: GALLERY (7 Cols Desktop, 1 Col Mobile) */}
          <div className="md:col-span-7">
            ...
            <div className="mt-8 hidden md:block">
              ...
              <div className="flex gap-4">
                <div className="w-24 h-32 md:w-32 md:h-40 bg-gray-100 border border-gray-300 flex items-center justify-center text-[10px] md:text-xs text-gray-400 text-center p-2">
                    CERTIFICATE SAMPLE
                </div>
                <div className="w-24 h-32 md:w-32 md:h-40 bg-gray-100 border border-gray-300 flex items-center justify-center text-[10px] md:text-xs text-gray-400 text-center p-2">
          ...
          {/* RIGHT: DETAILS & FORM (5 Cols Desktop, 1 Col Mobile) */}
          <div className="md:col-span-5">
            ...
            <div className="flex flex-col gap-3 mb-8">
              <button 
                ...
                className="w-full bg-[#25d366] hover:bg-[#20b85c] text-white py-4 px-6 rounded font-bold text-lg flex items-center justify-center gap-2 transition shadow-sm"
              >
                {/* SVG Icon */}
              ...
              <button 
                ...
                className="w-full bg-black text-white py-4 px-6 rounded font-bold text-lg hover:bg-gray-800 transition shadow-sm"
              >
                Buy Now
              </button>
            </div>
            ...
            <div className="bg-white border border-gray-200 p-6 rounded-lg shadow-sm mb-8">
              <h3 className="font-serif text-lg mb-4 text-gray-800">Make an Inquiry</h3>
              <form onSubmit={handleFormSubmit} className="space-y-4">
                <div>
                  ...
                  <input 
                    type="text" 
                    required 
                    className="w-full border border-gray-300 p-3 rounded text-sm focus:border-black outline-none"
                    placeholder="John Doe"
                    onChange={(e) => setFormData({...formData, name: e.target.value})}
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold uppercase text-gray-500 mb-1">Phone / Email</label>
                  <input 
                    type="text" 
                    required 
                    className="w-full border border-gray-300 p-3 rounded text-sm focus:border-black outline-none"
                ...
                <div>
                  ...
                  <textarea 
                    className="w-full border border-gray-300 p-3 rounded text-sm focus:border-black outline-none"
                    rows={3}
                    placeholder="I am interested in this stone..."
                    onChange={(e) => setFormData({...formData, message: e.target.value})}
                  ></textarea>
                </div>
                <button type="submit" className="w-full bg-gray-100 text-black border border-black py-3 font-bold uppercase text-sm hover:bg-gray-200 transition">
                  Submit Inquiry
                </button>
              </form>
            </div>
